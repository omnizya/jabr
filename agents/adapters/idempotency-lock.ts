...
export class IdempotencyLock {
  ...
  constructor(config: IdempotencyLockConfig = {}) {
    this.ttlMs = config.ttlMs ?? (Number(process.env.JABR_WEBHOOK_IDEMPOTENCY_TTL_MS) || 86_400_000);
  }
  ...
  acquire(eventId: string): LockResult {
    ...
    if (storedAt === undefined) {
      ...
      return { acquired: true, acquiredAt, expiresAt, ttlRemainingMs: this.ttlMs };
    }
  ...
  getSnapshot(): Record<string, { acquiredAt: string; expiresAt: string; ttlRemainingMs: number }> {
    this.gc();
    const now = performance.now();
    const out: Record<string, { acquiredAt: string; expiresAt: string; ttlRemainingMs: number }> = {};
    for (const [key, storedAt] of this.locks) {
 ...
 */
export function idempotencyConflictResponse(eventId: string): { status: number; body: object; headers: Record<string, string> } {
  return {
    status: 409,
    body: {
      jsonrpc: "2.0",
