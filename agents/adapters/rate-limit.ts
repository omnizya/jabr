...
export class RateLimiter {
  ...
  constructor(config: RateLimitConfig = {}) {
    this.windowMs = config.windowMs ?? (Number(process.env.JABR_RATE_LIMIT_WINDOW_MS) || 60_000);
    this.maxRequests = config.maxRequests ?? (Number(process.env.JABR_RATE_LIMIT_MAX_REQUESTS) || 60);
    this.resolveCaller = config.resolveCaller ?? this.resolveCallerDefault.bind(this);
  }
  ...
  getSnapshot(): Record<string, { used: number; cap: number; resetAt: string }> {
    const now = performance.now();
...
export function rateLimitResponse(retryAfterMs: number): { status: number; body: object; headers: Record<string, string> } {
  return {
    status: 429,
    body: {
      jsonrpc: "2.0",
      id: null,
      error: { code: -32002, message: "Rate limit exceeded. Retry after the indicated window.", data: { retryAfterMs, retryAfterSec: Math.ceil(retryAfterMs / 1000) } },
    },
    headers: { "Content-Type": "application/json", "Retry-After": String(Math.ceil(retryAfterMs / 1000)), "X-RateLimit-Remaining": "0" },
  };
}
