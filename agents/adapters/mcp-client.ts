...
export class McpClientAdapter implements McpToolPort {
  ...
  constructor(elicitationPort: ElicitationPort = NullElicitationPort) {
    this.elicitationPort = elicitationPort;
  }

  /**
   * Override the elicitation port after construction (e.g. wire a real gateway
   * port in CLI mode, keep NullElicitationPort in single-query mode).
   */
  ...
  private async handleElicitation(
    ...
    const elicitationRequest: ElicitationRequest = {
      mode,
      message: p.message || "",
      requestedSchema: p.mode === "form" ? (p.requestedSchema as Record<string, unknown> | undefined) : undefined,
      url: p.mode === "url" ? (p.url as string | undefined) : undefined,
      elicitationId: p.mode === "url" ? (p.elicitationId as string | undefined) : undefined,
    };

    const timeoutMs = 300_000; // 5 min — mirrors Hermes' MCP elicitation default
    const decision = await this.elicitationPort.elicit(elicitationRequest, timeoutMs);
