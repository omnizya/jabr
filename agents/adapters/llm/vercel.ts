...
export class VercelLlmAdapter implements LlmPort {
  ...
  constructor(
    ...
    const apiKey =
      opts?.apiKey ?? process.env.VERCEL_AI_GATEWAY_KEY ?? process.env.AI_GATEWAY_API_KEY ?? "";
  ...
  private callOptions(request: LlmRequest) {
    return {
      ...
      temperature: validateTemperature(request.temperature) ?? DEFAULT_MODEL_TEMPERATURE,
  ...
  async generate(request: LlmRequest): Promise<LlmResponse> {
    try {
      ...
      throw new Error(`[VercelLlmAdapter] generate failed: ${(e as Error).message}`);
    }
  }
  ...
  async streamGenerate(request: LlmRequest, onChunk: (chunk: string) => void): Promise<LlmResponse> {
    try {
      ...
      throw new Error(`[VercelLlmAdapter] streamGenerate failed: ${(e as Error).message}`);
    }
  }
}
