...
export class MemPalaceAdapter implements KnowledgePort {
  ...
  async store(slug: string, content: string, tags: string[], relations: string[] = []): Promise<void> {
    ...
    if (await file.exists()) {
      try {
        ...
        existingRelations = Array.from(new Set([...(existing.relations || []), ...relations]));
      } catch {
  ...
  async relate(slugA: string, slugB: string, relation: string): Promise<void> {
    ...
    if (entryA) {
      ...
      if (!entryA.relations.includes(rel)) {
        ...
        await this.store(entryA.slug, entryA.content, entryA.tags, entryA.relations);
      }
    }
    ...
    if (entryB) {
      ...
      if (!entryB.relations.includes(relInverse)) {
        ...
        await this.store(entryB.slug, entryB.content, entryB.tags, entryB.relations);
      }
    }
  }
  ...
  async query(text: string, topK = 3): Promise<KnowledgeEntry[]> {
    ...
    const scored = all.map((entry) => {
      ...
      for (const token of tokens) {
        ...
        const matches = (contentLower.match(new RegExp(`\\b${token}\\b`, "g")) || []).length;
        score += matches;
      }
