...
const stubSkills: SkillStorePort = { save: () => false, exists: () => false, list: () => [] };
...
async function runOracle(llmText: string | null, taskText: string): Promise<string> {
  const store = new TaskMemory();
  const agent = new OracleAgent(store, stubSkills, llmText === null ? undefined : stubLlm(llmText));
...
describe("OracleAgent LLM-driven routing", () => {
  test("emits %%HANDOVER%% with valid transferTo when LLM says handover", async () => {
    const text = await runOracle(
      JSON.stringify({ decision: "handover", transferTo: "fixer", reason: "bug fix", context: "fix the bug in checkout" }),
  ...
  test("answers directly when LLM says answer", async () => {
    const text = await runOracle(JSON.stringify({ decision: "answer" }), "review this function for edge cases");
    expect(text).toContain("## Code Review");
    expect(text).not.toContain(HANDOVER_MARKER);
  });

  test("falls back to keyword answer on non-JSON LLM output", async () => {
    const text = await runOracle("I think this is a review task, let me answer it.", "review this function");
    expect(text).toContain("## Code Review");
    expect(text).not.toContain(HANDOVER_MARKER);
  });

  test("falls back to keyword answer when transferTo is invalid", async () => {
    const text = await runOracle(JSON.stringify({ decision: "handover", transferTo: "not-an-agent" }), "simplify this code");
