...
function prPayload(): string {
  return JSON.stringify({
    ...
    sender: { login: "octocat", id: 42, avatar_url: "https://github.com/images/error/octocat_happy.gif" },
    action: "opened",
    pull_request: {
      number: 42,
...
describe("6 · GitHub PR → Oracle review → comment (integration)", () => {
  ...
  beforeAll(async () => {
    ...
    delegateServer = Bun.serve({
      ...
      async fetch(req) {
        ...
        let raw: string;
        try { raw = await req.text(); } catch { return new Response("bad request", { status: 400 }); }
        let body: unknown;
    ...
    if (!adapterPort) throw new Error("GitHubWebhookAdapter did not bind a port");
  });
  ...
  test("adapter delegates PR to delegateUrl with correct A2A payload", async () => {
    ...
    const envelope = lastCall.body as { jsonrpc?: string; method?: string; params?: { message?: { parts?: Array<{ kind: string; text: string }> } } };
    expect(envelope.jsonrpc).toBe("2.0");
    expect(envelope.method).toBe("tasks/send");
    const text = envelope?.params?.message?.parts?.[0]?.text ?? "";
    expect(text).toContain("GitHub PR #42 opened on omnizya/jabr: Add e2e webhook test");
  ...
  test("createComment without token logs a warning and returns without throwing", async () => {
    ...
    console.warn = (...args: unknown[]) => {
      if (typeof args[0] === "string" && args[0].includes("createComment")) warned = true;
      warn(...args);
    };
    try {
