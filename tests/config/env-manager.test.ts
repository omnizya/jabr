...
describe("requireEnv", () => {
	...
	test("logs loaded value", () => {
		...
		expect(captured.info.some((m) => m.includes("JABR_X402_HMAC_SECRET"))).toBe(
			true,
		);
	});
...
describe("requireIntEnv", () => {
	...
	test("enforces min bound", () => {
		...
		expect(() => requireIntEnv("ORCHESTRATOR_PORT", { min: 1024 })).toThrow(
			EnvVarError,
		);
	});
	...
	test("enforces max bound", () => {
		...
		expect(() => requireIntEnv("ORCHESTRATOR_PORT", { max: 65535 })).toThrow(
			EnvVarError,
		);
	});
});
...
describe("optionalJsonEnv", () => {
	...
	test("parses valid JSON", () => {
		...
		expect(optionalJsonEnv<{ key: string }[]>("A2A_API_KEYS", [])).toEqual([
			{ key: "abc" },
		]);
	});
	...
	test("returns fallback when unset", () => {
		expect(
			optionalJsonEnv<{ key: string }[]>("A2A_API_KEYS", [{ key: "default" }]),
		).toEqual([{ key: "default" }]);
	});
