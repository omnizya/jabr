...
import { parseWhatsAppEvent, WhatsAppWebhookAdapter } from "@adapters/http/whatsapp-webhook";
import type { WhatsAppWebhookEvent, WhatsAppTextMessage } from "@ports/whatsapp-bot-port";
...
async function makeTestAdapter(
  ...
  const { WhatsAppWebhookAdapter } = await import("@adapters/http/whatsapp-webhook");
  ...
  return {
    route: (event: WhatsAppWebhookEvent) =>
      (adapter as unknown as { route: (e: WhatsAppWebhookEvent) => Promise<void> }).route(event),
    get fetchCalls() {
      return fetchCalls;
    },
    clearFetchCalls: () => {
...
describe("parseWhatsAppEvent", () => {
  test("parses a text message event", () => {
    ...
    expect((change.value.messages![0]! as WhatsAppTextMessage).text.body).toBe("fix bug");
  ...
  test("parses a text message with a longer body", () => {
    ...
    expect((msg as WhatsAppTextMessage).text.body).toBe("fix this bug in the parser");
...
describe("WhatsAppWebhookAdapter delegation", () => {
  ...
  test("routes a text message → delegateUrl with tasks/send", async () => {
    ...
    expect(body.params.message.parts[0]!.text).toContain("[WhatsApp] From ++212612345678:");
  ...
  test("does not delegate when delegateUrl is empty", async () => {
    ...
    await (noDelegate as unknown as { route: (e: WhatsAppWebhookEvent) => Promise<void> }).route(event);
...
describe("WhatsAppWebhookAdapter signature verification", () => {
  ...
  test("POST /webhook with valid signature → 200", async () => {
    ...
    const delegateCall = fetchCalls.find(
      (c) => c.url.includes("localhost:4005"),
    );
