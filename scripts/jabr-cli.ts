...
function warn(...args: unknown[]) { console.warn(...args); }
function error(...args: unknown[]) { console.error(...args); }
...
async function killAgentByPort(port: number): Promise<void> {
  if (port === 0) return;
  spawnSync("bash", ["-c", `lsof -ti :${port} 2>/dev/null | xargs kill 2>/dev/null || true`], { encoding: "utf-8" });
}

/** Check if an HTTP endpoint responds (for port-based agents). */
async function healthCheck(name: string, port: number): Promise<{ up: boolean; card?: object }> {
...
async function startSingle(agent: typeof AGENTS[number]) {
  ...
  if (agent.port > 0) {
    ...
    if (ready) {
      ...
      warn(`  ? ${agent.name} — launched but not yet serving on port ${agent.port} (check ${logPath})`);
    }
  } else {
    ...
    setTimeout(() => {
      ...
      if (procPids.length > 0) {
        ...
        warn(`  ? ${agent.name} — process may not have started (check ${logPath})`);
      }
    }, 1500);
  }
}
...
async function stopSingle(agent: typeof AGENTS[number]) {
  ...
  if (agent.port > 0) {
    ...
    if (stillUp.up) {
      warn(`  ✗ ${agent.name}: process killed but port ${agent.port} still serving`);
    } else {
...
async function cmdStatus() {
  ...
  log("");
  log(`Summary: ${httpAgents} HTTP + ${stdioAgents} stdio agents | ${runningCount}/${AGENTS.length} running`);
}
...
async function cmdLogs(args: string[]) {
  ...
  if (target && target !== "all") {
    ...
    if (!existsSync(logPath)) {
      ...
      log(`  Start the agent first: bun scripts/jabr-cli.ts start ${agent.name}`);
      return;
    }
...
async function cmdSend(args: string[]) {
  ...
  if (agent.port === 0) {
    error(`${agent.name} is stdio-based — cannot send via HTTP. Use the orchestrator instead.`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (res.ok && body.result) {
      const resultText = (body.result as any).text ?? JSON.stringify(body.result);
    ...
    error(`Is the agent running? Start it with: bun scripts/jabr-cli.ts start ${agent.name}`);
  }
}
