...
sequence of commands. This is a fundamental requirement of the platform's replay mechanism:
when a worker restarts or a workflow is recovered, the platform re-executes the workflow code
...
class MyWorkflow:
    ...
    async def run(self, params: MyParams) -> MyResult:
        # ✅ Correct - call the activity directly; non-deterministic work runs inside it
...
When determinism enforcement is enabled, your workflow code runs inside the SDK's determinism sandbox. The sandbox:
...
When enabled, the SDK runs the workflow inside its determinism sandbox with the standard restrictions; no custom sandbox policy is applied.
...
class MyWorkflow:
    ...
    async def run(self, input: str) -> str:
        # This code runs WITHOUT the determinism sandbox.
...
Both context managers are exposed on `workflow.unsafe`, so you never import `temporalio` directly.
