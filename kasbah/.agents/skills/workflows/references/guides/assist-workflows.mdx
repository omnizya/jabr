...
To publish a conversational workflow as an assistant in Vibe (Mistral's chat interface), your workflow must return a `ChatAssistantWorkflowOutput`.
The output will not be displayed in Vibe, but we enforce a common interface for inter-operability.
