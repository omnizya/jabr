...
All patterns are built on the platform's workflow continuation primitives, providing automatic fault tolerance, progress tracking, and scalability for large datasets.

### Key Benefits

- **Massive Parallelism**: Execute thousands of activities concurrently
- **Automatic Continue-As-New**: Handle large datasets without hitting the execution history limits
...
- **Payload limits**: Remember the 2MB input/output limit per activity
...
- Track progress with the observable `task()` / `task_from()` context managers
...
- [Limitations Guide](limitations) - Understand the platform's execution constraints
