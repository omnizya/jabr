...
### Catching Workflow and Activity Errors

In user code, catch the SDK's own exception types — never import `temporalio`. Both are exported from `mistralai.workflows`:

```python
from mistralai.workflows import WorkflowError, ActivityError

try:
    result = await client.workflows.execute_workflow_and_wait_async(...)
except ActivityError as e:
    # An activity failed after exhausting its retries
    ...
except WorkflowError as e:
    # The workflow itself raised an application error
    ...
```

- `ActivityError` wraps a failure raised inside an activity.
- `WorkflowError` is the base class for application errors raised by workflow code.
