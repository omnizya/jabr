...
- **Keep orchestration code fast**: CPU-bound logic in the workflow body (between `await` points) should stay under ~2 seconds per step — push all heavy or blocking work into activities
...
> **What is a task queue?** A task queue is a lightweight routing mechanism that determines which workers can execute which workflows and activities. Workers poll specific task queues for work, and workflows/activities are assigned to task queues when executed. A worker's task queue is its `DEPLOYMENT_NAME`.
...
- Different deployment names (task queues) for isolation
- Use case: Testing changes without affecting running workers

```bash
## Stable version
DEPLOYMENT_NAME=my-workflow MISTRAL_API_KEY=workspace_key uv run python my_workflow.py

## Testing changes
DEPLOYMENT_NAME=my-workflow-testing MISTRAL_API_KEY=workspace_key uv run python my_workflow_v2.py
```
...
- For testing changes → Use different deployment names (task queues) on the same workspace
- For scaling → Use same workspace, same deployment name, same code
...
- **Action endpoints target latest run**: This ensures you always interact with the current state
