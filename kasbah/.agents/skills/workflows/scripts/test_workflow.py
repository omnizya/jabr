...
def _select_entrypoint(
    ...
    With ``name_override`` pick the matching registered name (or None if absent).
    Otherwise, when several workflows exist, the entrypoint is the one *not* used as
    a child via ``execute_workflow(...)`` — so a parent runs, not its sub-workflow.
    Falls back to the first class when ambiguous.
    """
    if name_override:
        for c in classes:
    ...
    candidates = [c for c in classes if c.__name__ not in child_names]
    if len(candidates) == 1:
        return candidates[0]
...
def _discover_workflow(workflow_file: Path, name_override: str | None) -> tuple[type, str, bool, list[type]]:
    ...
    Returns ``(entrypoint_cls, name, interactive, all_classes)``. *all_classes* is
    every discovered @workflow.define class — they must all be registered so a
    parent's child sub-workflows can run.
    """
    ...
    cls = _select_entrypoint(found, workflow_file.read_text(), name_override, _workflow_name)
    if cls is None:
...
async def run_workflow(
    ...
    worker_task = await workflows.run_worker(all_classes, detach=True)
