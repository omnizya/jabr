...
/**
 * Parse a text/event-stream body into structured SSE event frames.
 *
 * Splits on double-newline boundaries (per WHATWG SSE spec), then extracts
 * `event:` and `data:` fields from each frame. The `data` field is JSON-parsed
 * when possible; otherwise it is returned as raw text.
 *
 * @param raw  The full SSE response body (e.g. from `Response.text()`).
 * @returns    An array of parsed `{ event, data }` frames.
 */
