...
export function runAgent(config: {
	...
	verbose(
		`[Serve] ${config.card.name}: verbose logging enabled (JABR_VERBOSE=true)`,
	);
	verbose(
		`[Serve] ${config.card.name}: streaming=${Boolean(config.onTaskStreaming)} requireAuth=true`,
	);
	...
	const server = new A2AServer({
		...
		onTask: async (text: string): Promise<string> => {
			...
			verbose(
				`[Serve] ${config.card.name}: executing task ${taskId} (${text.length} chars)`,
			);
			taskStore.create(taskId);
			await config.execute(taskId, text);
			const result = format(taskStore, taskId);
			verbose(
				`[Serve] ${config.card.name}: task ${taskId} completed in ${Date.now() - started}ms`,
			);
			return result;
		},
