...
import { startBunWebSocketAdapter } from "@adapters/websocket/bun-websocket-adapter.ts";
...
if (import.meta.main) {
	...
	const realtime: RealtimePort = startBunWebSocketAdapter({
		port: JABR_PORTS.realtime,
	});
	...
	for (const [name, url] of Object.entries(seedUrls)) {
		...
		verbose(
			`[Run:Orchestrator] ${card.name}: ${card.name} card ok (${url}), pricing.settlement=${card.pricing?.settlement !== undefined}`,
		);
	...
	const server = new A2AServer(
		{
			...
			async onTask(text: string, caller?: ResolvedCaller): Promise<string> {
				...
				verbose(
					`[Run:Orchestrator] executing task ${taskId} (${text.length} chars) caller=${caller?.description ?? "unknown"}`,
				);
				...
				verbose(
					`[Run:Orchestrator] task ${taskId} completed in ${Date.now() - started}ms`,
				);
				return (
