...
export function requireIntEnv(
  ...
  const raw = process.env[name];
  if (raw === undefined || raw === "") {
    throw new EnvVarError(name, `Fatal: ${name} environment variable is required (expected integer)`);
  }
  const n = Number(raw);
  if (!Number.isInteger(n) || n <= 0) {
    throw new EnvVarError(
  ...
  if (opts.min !== undefined && n < opts.min) {
    throw new EnvVarError(name, `Fatal: ${name}=${n} is below minimum ${opts.min}`);
  }
  if (opts.max !== undefined && n > opts.max) {
    throw new EnvVarError(name, `Fatal: ${name}=${n} exceeds maximum ${opts.max}`);
  }
...
export function requireUrlEnv(name: string): string {
  const raw = process.env[name];
  if (raw === undefined || raw === "") {
    throw new EnvVarError(name, `Fatal: ${name} environment variable is required`);
  }
  try {
    const u = new URL(raw);
...
export function requireJsonEnv<T>(name: string): T {
  const raw = process.env[name];
  if (raw === undefined || raw === "") {
    throw new EnvVarError(name, `Fatal: ${name} environment variable is required (expected JSON)`);
  }
  try {
    return JSON.parse(raw) as T;
  } catch (e) {
 ...
 * env.url("NINEROUTER_URL", { default: "http://127.0.0.1:20128" });
